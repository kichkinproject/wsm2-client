...
@Component({
  ...
  templateUrl: './integrator-edit.component.html',
  styleUrls: ['./integrator-edit.component.scss']
})
export class IntegratorEditComponent implements AfterViewInit {
  ...
  public accessed() {
    ...
    return role === Roles.MAIN_ADMIN || role === Roles.ADMIN || role === Roles.INTEGRATOR;
  }
  ...
  public saveIntegrator() {
    const integrator = this.dataService.getIntegrator(this.integratorLogin);
    this.dataService.updateIntegrator(integrator.login, this.$login, integrator.password, this.$name, this.$info);
